...
const EventHistory = ({ selectedApp }) => {
    ...
    return (
        <div class="border shadow-md w-1/2 my-4 mx-8 p-4 rounded-md bg-[#FFFFFF] ml-auto">
            ...
            <div class="bg-[#EBEBEB] h-[0.75px]"></div>
            {(filteredData && !showAll) && (
                <>
                    {filteredData.slice(0, 4).map((item, index) => (
                        <div key={item.id} class="flex justify-between px-4 py-2 items-center">
                            <div class="w-1/3">
                                <h1 class="text-sm">{item.event}</h1>
                                <h2 class="text-xs text-[#A5A5A5]">{convertTimestampToHours(item.timestamp)} hours ago</h2>
                            </div>
                            <div class="text-sm w-1/3">
                                {item.version}
                            </div>
                            <div class="w-1/3">
                            <div class={`flex w-28 items-center text-[14px] border rounded-md pr-1 ${getStatusColor(item.status)}`}>
                    ...
                    <button class="text-[#6E27D5] underline text-sm font-semibold px-4 py-2" onClick={handleViewMoreLess}>
                        View More
                    </button>
                </>
            )}
