...
const ServiceMetricsCard = () => {
  ...
  return (
    <div className="border shadow-md w-1/2 my-4 mx-8 p-4 rounded-md bg-white">
    <h1 className="text-[#595959] text-[16px] font-bold">Service metrics</h1>
    <div className="flex justify-around text-sm text-black font-semibold px-4 pt-4 pb-2">
        <div className={`cursor-pointer text-sm ${selectedGraph==="CPU"?'text-[#6E27D5]':'text-[#595959]'} font-bold`} onClick={handleCPUButtonClick}>
            CPU
        </div>
        <div className={`cursor-pointer text-sm ${selectedGraph==="Memory"?'text-[#6E27D5]':'text-[#595959]'} font-bold`} onClick={handleMemoryButtonClick}>
            Memory
        </div>
    </div>
    {selectedGraph==="Memory"?<div className="bg-[#6E27D5] h-[1px] w-1/2 ml-auto"></div>:<div className="bg-[#6E27D5] h-[1px] w-1/2"></div>}
    <div className="bg-[#EBEBEB] h-[1px]"></div>
